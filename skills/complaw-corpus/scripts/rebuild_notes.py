...
def main():
    ...
    last = 0
    built = pages_touched = restarts = 0
    sample = []
    for pg in pages:
        nl = pg.get("note_lines") or []
        if not nl:
            continue
        notes, last = _parse_page(nl, marker, last, pg)
        if not notes:
            # Счёт сносок начинается заново в каждой главе. Сквозной
            # счётчик после первой же главы отвергает всё: «26 > 431»
            # ложно. Если полоса с аппаратом не дала НИ ОДНОЙ сноски,
            # пробуем ещё раз, считая с нуля: внутри одной полосы номера
            # всё равно возрастают. У von Bar/Drobnig, где каждый
            # национальный очерк нумеруется сам по себе, это разница
            # между 196 сносками и 1516.
            notes, last = _parse_page(nl, marker, 0, pg)
            if notes:
                restarts += 1
    ...
    if restarts:
        print(f"полос, где счёт пришлось начать заново (новая глава): {restarts}")
    for pno, ns in sample:
        print(f"  f{pno}: {ns}")
    if a.dry_run:
...
def _parse_page(nl, marker, last, pg):
    """Разобрать аппарат одной полосы. Возвращает список сносок и новый счётчик."""
    notes, cur = [], None
    for ln in nl:
