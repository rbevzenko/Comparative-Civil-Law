...
def reordered(ln):
    """Слова строки лежат не слева направо — значит, их переставили нарочно.

    move_margin_numbers.py переносит маргинальный номер абзаца с правого
    поля в начало строки, сохраняя слову исходную геометрию. Пересборка
    такой строки из символов PDF (а они идут по x0) вернула бы номер в
    конец и стёрла адрес абзаца. Такие строки не трогаем.
    """
    xs = [w["x0"] for w in ln.get("words") or []]
...
def main():
    ...
    if skipped:
        print(f"пропущено с переставленными словами: {skipped}")
