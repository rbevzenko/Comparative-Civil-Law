...
mcp = FastMCP(
    ...
    instructions=(
        ...
        '«Comparative law» к национальной юрисдикции не привязана: это значение поля jurisdiction пишется полностью, а не кодом. Какие из этих значений реально наполнены сегодня — только из corpus_stats.\n'
        ...
        'Ответ по вопросу, затрагивающему несколько юрисдикций, строится так: кластер → юрисдикция → слой. Для «Comparative law» деления по слоям нет: кластер → материал. Слой, по которому ничего не найдено, не опускается, а помечается (см. п. 2).\n'
        '\n'
