...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "страницу с этим конкретным фрагментом и его реквизитами. ВСЕГДА "
        "делай ссылку прямо из самой цитаты: оформляй её как markdown-"
        "ссылку [текст цитаты](universal_ref), а не отдельным голым URL "
        "рядом с текстом. Так же оформлены ссылки на источник в других "
        "коннекторах контура (КонсультантПлюс, «Высшие суды» и т.п.)."
    ),
    stateless_http=True,
    json_response=True,
...
async def search_corpus(
    ...
    реквизитами. ВСЕГДА оформляй саму цитату как markdown-ссылку на
    universal_ref — [цитата](universal_ref) — а не отдельным голым URL
    рядом с текстом.
    """
...
async def get_chunk(chunk_id: str) -> str:
    ...
    ВСЕГДА оформляй саму цитату как markdown-ссылку на universal_ref —
    [цитата](universal_ref) — а не отдельным голым URL рядом с текстом.
    """
    try:
