...
def dedup_chars(chars, dx, dy):
    ...
    Допуск задаётся числом, а не долей от ширины знака. Долю пробовали:
    сдвиг второго слоя вроде бы должен расти с кеглем, но на деле он почти
    постоянен — у Урлиака двойники стоят в пределах 0.1–1.1 пункта по
    горизонтали независимо от кегля. Доля 0.45 ширины давала на кегле тела
    допуск в 1.8 пункта и оставляла часть двойников; поднять её до
    заголовочного значения нельзя — на теле она начинает съедать настоящие
    удвоенные буквы («at ribution» вместо «attribution»).

    Рабочее значение для Урлиака — 3.0 по обеим осям: доля подозрительных
    слов после чистки 0.3% против сплошного «HHIISSTOTIOREI RE» до неё.
    """
    kept = []
    buckets = {}
    step = max(dx, 0.1)
