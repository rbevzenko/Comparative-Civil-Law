...
пагинацией с единицы и со своей нумерацией сносок с единицы. Внутри куска
порядок всегда один: сначала текст, потом весь аппарат. Поэтому границей
служит первая строка «1. », за которой в том же куске идёт «2. » — дальше
до конца куска всё сноски, сколько бы полос они ни занимали.
...
def main():
    ...
    for chunk in chunks:
        ...
        for pos, (i, n) in enumerate(nums):
            if n != 1:
                continue
            if any(m == 2 for _, m in nums[pos + 1:]):
                start = i
                break
        if start is None:
            continue
