...
`--require-number` добавляет условие для хвоста, начинающегося ПОД текстом:
блок сносок открывается номером. Без него в аппарат уезжает таблица, если
она набрана тем же мелким кеглем и стоит в конце полосы.
"""
...
def main():
    ...
    ap.add_argument("--require-number", action="store_true",
                    help="хвост под текстом принимать, только если он открывается номером сноски")
    ...
    for pg in pages:
        ...
        if len(tail) < a.min_lines:
            continue
        # Блок сносок, начинающийся под текстом, открывается номером сноски.
        # Без этой проверки в аппарат уезжает таблица, набранная тем же
        # мелким кеглем и стоящая в конце полосы: у Burrows, Obligations так
        # пропала «Table 4.2 Purposes of main judicial remedies». На полосе,
        # целиком набранной мелким кеглем, проверять нечего — там хвост
        # продолжает сноску с предыдущей полосы.
        if a.require_number and cut > 0 and not tail[0]["text"].strip()[:1].isdigit():
            continue
