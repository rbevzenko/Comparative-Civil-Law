...
ARROW = {"(>": "→", ">": "→", "+": "→", "—": "→", "-": "→", "»": "→"}
"""Стрелка перекрёстной ссылки нарисована контуром, а скобка перед ней —
обычный текст. Поэтому «(>» распознавания заменяется одной стрелкой:
скобку иначе получим дважды."""
...
def page_patches(args):
    ...
    # Один поток на tesseract. По умолчанию он берёт OpenMP и разворачивается
    # на все ядра; когда таких процессов несколько, они дерутся за одни и те
    # же ядра, и полоса считается не две секунды, а без малого три минуты —
    # замерено на этом самом файле. Параллелизм даёт пул, а не сам tesseract.
